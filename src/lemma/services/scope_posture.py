...
- ``evidenced`` — have at least one inbound ``EVIDENCES`` edge (real-world
  evidence ties back to them).
- ``covered`` — both.
...
def compute_posture(scope_name: str, graph: ComplianceGraph) -> ScopePosture:
    ...
    # Build inbound SATISFIES and EVIDENCES sets per control for O(1) lookup.
