...
- `github` — collects branch protection on `main`, CODEOWNERS presence, open Dependabot alert counts bucketed by severity, and **Actions workflow-token permissions** (the supply-chain hardening OpenSSF Scorecard's Token-Permissions check covers — `GET /repos/<repo>/actions/permissions/workflow`; `status_id=1` only when the default `GITHUB_TOKEN` permission is `read` and Actions cannot approve pull requests, `2` when over-permissioned, `0` when Actions is disabled; `metadata.{default_workflow_permissions, can_approve_pull_request_reviews}`). Auth via the `LEMMA_GITHUB_TOKEN` environment variable (optional for public repos within GitHub's 60-req/hr unauthenticated cap; required for private repos and to lift the rate limit to 5000/hr). Rate-limited responses raise a clean error naming the endpoint.
- `okta` — collects identity-governance posture as six events per run: MFA enrollment policy state, the SSO application inventory (active vs. total), the **user-lifecycle inventory** (`metadata.{active_count, suspended_count, deprovisioned_count, total_count}`), the **group/admin roster** (`metadata.{group_count, admin_group_count, admin_groups}`, where a group counts as privileged when its name contains `admin`), the **password policy** (`status_id=1` when an active policy exists, `2` when none), and a 24h **authentication-events** rollup emitted as an OCSF `AuthenticationEvent` (class_uid 3002) with `metadata.{success_count, failure_count, window_hours}` read from the Okta System Log API. Auth via the `LEMMA_OKTA_TOKEN` environment variable (required — Okta has no unauthenticated API). The token is passed as an `SSWS <token>` authorization header. Rate-limited responses (HTTP 429) raise a clean error naming the endpoint; any individual signal whose API call fails degrades to `status_id=0` (Unknown) rather than aborting the run. Stable `metadata.uid` per `(signal, domain, UTC date)` so same-day re-runs dedupe against themselves.
- `aws` — collects account-level posture across **five services** aligned with the CIS AWS Foundations Benchmark, eight events per run: IAM root-account MFA (CIS 1.5), IAM password policy presence + minimum length ≥ 14 (CIS 1.8), **IAM user/access-key inventory** (paginated `list_users` so large accounts aren't capped; flags access keys older than 90 days via `metadata.{user_count, stale_access_key_count}`), at least one multi-region CloudTrail (CIS 3.1), **AWS Config recorder** recording (`metadata.{recorder_count, recording_count}`), **account-level S3 Block Public Access** all four flags enabled (`metadata.block_*`), **VPC flow logs** present on every VPC (`metadata.{vpc_count, vpcs_with_flow_logs}`), and **default security-group lockdown** (CIS 5.3 — default SGs should carry no rules; `metadata.{default_sg_count, open_default_sg_count}`). Auth via the AWS default credential chain (env vars, AWS profile, IMDS) — no Lemma-specific env var; the connector queries read-only describe/get APIs across `iam`, `cloudtrail`, `config`, `s3control`, and `ec2`. Missing credentials raise a clean error at construction time rather than mid-collect. Stable `metadata.uid` per `(signal, account_id, UTC date)` so same-day re-runs dedupe against themselves.
- `jira` — collects change-management posture from a Jira Cloud site as the auditable signal a SOC 2 CC8.1 review wants for change management. Emits one `ComplianceFinding` aggregating the approved / rejected breakdown across recent change tickets (default JQL: `labels = "change-management" ORDER BY created DESC`; override via the `jql` config field). Auth via HTTP Basic with `email:api_token`; the API token is required (`LEMMA_JIRA_TOKEN` env var). Rate-limited responses (HTTP 429) raise a clean error naming the endpoint. Stable `metadata.uid` per `(site, UTC date)` so same-day re-runs dedupe against themselves. Use the config-file mode: `connector: jira` with `config: {base_url, email, jql?}`.
- `azure` — collects tenant + subscription posture from Microsoft Azure: Entra ID conditional-access MFA (at least one ENABLED conditional-access policy with MFA in `grantControls.builtInControls`), Azure Activity Log diagnostic settings (presence of any setting on `Microsoft.Insights/diagnosticSettings`), and Azure Policy assignments (count of assignments on `Microsoft.Authorization/policyAssignments`). Auth via OAuth2 client-credentials flow against `https://login.microsoftonline.com/<tenant_id>/oauth2/v2.0/token`; the secret is required (`LEMMA_AZURE_CLIENT_SECRET` env var or `client_secret=...` constructor arg). Two scopes are negotiated: `https://graph.microsoft.com/.default` for the Entra ID call and `https://management.azure.com/.default` for the Resource Manager calls. Tokens are cached per scope on the connector instance, so a single run hits the token endpoint at most twice. Status mapping per finding: `status_id=1` (Pass) when the signal is present, `status_id=2` (Fail) when the signal is absent, `status_id=0` (Unknown) when the upstream call fails. 429 responses raise a clean error naming the endpoint. Stable `metadata.uid` per `(tenant_id, subscription_id, signal_type, UTC date)`. Use the config-file mode: `connector: azure` with `config: {tenant_id, client_id, subscription_id}` and the secret in the env.
- `servicenow` — collects change-request posture from a ServiceNow instance — the SOC 2 CC8.1 audit signal mirrored from the Jira connector for orgs running ServiceNow as their ITSM of record. Emits one `ComplianceFinding` aggregating approved / rejected / other counts across recent records in the `change_request` table. Default query is empty (every record); operators override via the `query` config field using ServiceNow's encoded query language (e.g. `state=3^assignment_group=devops`). Auth via HTTP Basic with `username:password`; the password is required (`LEMMA_SERVICENOW_PASSWORD` env var). State buckets: approved = `{Closed Complete, Closed Successful, Approved}`; rejected = `{Closed Cancelled, Closed Unsuccessful, Rejected, Closed Skipped}`; everything else (in-flight) lands in `change_other`. 429 responses raise a clean error naming the endpoint. Stable `metadata.uid` per `(instance, UTC date)`. Use the config-file mode: `connector: servicenow` with `config: {instance, username, query?}`.
- `azure-devops` — collects change-management posture from an Azure DevOps project's work items — same SOC 2 CC8.1 audit signal as the Jira and ServiceNow connectors, on the same OCSF wire format so cross-tool aggregation in `lemma control-plane compliance` groups findings from all three under the same `(framework, control_id)` key. Two-step REST call: `POST /<org>/<project>/_apis/wit/wiql` returns matching work-item IDs, then `GET /<org>/_apis/wit/workitems?ids=...&fields=System.Id,System.State` returns each item's state. Default WIQL: `SELECT [System.Id], [System.State] FROM workitems WHERE [System.Tags] CONTAINS 'change-management'`; override via the `wiql` config field for per-area-path / per-sprint / per-process scoping. Auth via Personal Access Token (PAT) over HTTP Basic with empty username + PAT as password (the canonical `Basic <base64(":" + PAT)>` form); PAT via `LEMMA_AZURE_DEVOPS_TOKEN` env var. State buckets (default-process names): approved = `{Closed, Done, Completed, Resolved}`; rejected = `{Removed, Cancelled}`; everything else (in-flight) lands in `change_other`. 429 responses raise a clean error naming the endpoint. Stable `metadata.uid` per `(organization/project, UTC date)`. Use the config-file mode: `connector: azure-devops` with `config: {organization, project, wiql?}`.
- `gcp` — collects project-level posture from a Google Cloud project: IAM service-account key age (count of user-managed keys + how many are older than the 90-day rotation window) and Cloud Audit Logs sink presence (count of project log sinks with destinations on BigQuery or Cloud Storage, the two destinations that give durable retention). Emits two `ComplianceFinding` events per run. Auth via a pre-minted OAuth2 access token passed as `access_token=...` or via the `LEMMA_GCP_ACCESS_TOKEN` env var; the token is sent as `Authorization: Bearer <token>`. Operators with a service-account JSON mint a token out-of-band (`gcloud auth print-access-token` or `google.oauth2.service_account.Credentials`) — keeps the connector stdlib + httpx-only and avoids the heavyweight `google-auth` dependency for v0. 429 responses raise a clean error naming the endpoint. Stable `metadata.uid` per `(project, signal_type, UTC date)` so same-day re-runs dedupe against themselves. Use the config-file mode: `connector: gcp` with `config: {project_id, access_token?}`.
...
`sign` reads one OCSF event from `--event <path>` or stdin, resolves the producer's ACTIVE key from `<dir>/<producer>/meta.json`, and emits one signed envelope JSONL line on stdout. Defaults to genesis (`"0"*64`) when `--prev-hash` is omitted; operators chain envelopes by piping the prior entry's `entry_hash` into the next call. The output is byte-compatible with Python's `EvidenceLog.append` — Python verifies what Go signs and vice versa.

`ingest` is the batch counterpart to `sign`: read OCSF events from a `.json` file (single object), `.jsonl` file (one object per line), or `-` for JSONL on stdin, sign each one with the producer's ACTIVE key, and append the resulting envelopes to `<evidence-dir>/<YYYY-MM-DD>.jsonl` files keyed on each event's `time`. The chain hash flows across the batch and across calls — second invocations pick up `prev_hash` from the latest entry on disk. Dedup is per-day and matches Python's `lemma evidence ingest` (skip when `metadata.uid` or content hash already appears in the day's file). Source-stage provenance is built once per call (`actor = "lemma-agent-ingest:<label>"`, `content_hash = sha256(raw input bytes)`); operators override the label with `--source-label`. Output: `<N> ingested, <M> skipped (duplicate).` on stdout, exit `0` even if every event was deduped.

**Caveat (sign and ingest)**: the input event must already be in the form Pydantic's OCSF models would emit (default fields like `severity_id`, `status_id`, `message` set explicitly), or the Python verifier will recompute a different canonical hash and reject the envelope. Pre-normalize through `lemma.services.ocsf_normalizer.normalize` if needed.

`keygen` mints an Ed25519 keypair for `--producer` under `--keys-dir` and writes the same on-disk layout Python's `crypto.generate_keypair` produces: `<safe_producer>/<key_id>.private.pem` (PKCS#8, mode 0o600), `<safe_producer>/<key_id>.public.pem` (SubjectPublicKeyInfo, 0o644), and `<safe_producer>/meta.json` with one ACTIVE record. The `key_id` derivation (`"ed25519:" + sha256(raw_public).hex()[:16]`) and `_safe_producer` rule (replace `/` and ` ` with `_`) match Python byte-for-byte, so a key minted by either side is loadable by both. **Idempotent** — a second call against a producer that already has an ACTIVE key returns its key_id without modifying any files.

`keyrotate` retires the producer's current ACTIVE key and mints a fresh one. The prior ACTIVE record is updated in-place to `status: RETIRED`, `retired_at: <now>`, and `successor_key_id: <new_key_id>`; a new ACTIVE record is appended. Mirrors Python's `crypto.rotate_key` on the wire. Errors when no ACTIVE key exists for the producer (nothing to rotate from). Output: `Rotated <producer>: <old_key_id> retired, <new_key_id> now ACTIVE.`

`keyrevoke` marks `<key-id>` as `status: REVOKED` with `revoked_at: <now>` and `revoked_reason: <text>`. Mirrors Python's `crypto.revoke_key`. `--reason` must be non-empty; the matching record must exist. Revocation is forward-looking — the verifier flips a future envelope to VIOLATED only when its `signed_at >= revoked_at`. Output: `Revoked <key_id> for producer <name>: <reason>`. With `keygen` + `keyrotate` + `keyrevoke` the agent now has full key-lifecycle parity with Python, so an air-gapped operator can deploy and operate a Lemma agent with no Python dependency.
...
`serve` runs an in-process HTTP server bound to `127.0.0.1:<port>` exposing two endpoints. `GET /health` returns a JSON snapshot derived from disk: `version`, `evidence_count` (non-blank lines across `*.jsonl` files in `--evidence-dir`), `last_signed_at` (latest `signed_at` across all envelopes), `producer_count` (subdirectories of `--keys-dir` that contain a `meta.json`), `started_at`, and `uptime_seconds`. `GET /metrics` returns the same state in Prometheus exposition format (`text/plain; version=0.0.4`) with three series: `lemma_agent_uptime_seconds` (gauge), `lemma_agent_evidence_total` (counter), `lemma_agent_producers` (gauge). `/health` is what `lemma agent status` queries; `/metrics` satisfies #25's "Prometheus-style federation" task. The server runs in the foreground and shuts down cleanly when stdin EOFs (or on `SIGTERM` in production).
...
- `GET /metrics` — Prometheus exposition format with `control_plane_uptime_seconds` (gauge), `control_plane_evidence_total` (counter, disk-derived; survives restart), `control_plane_producers` (gauge), and `control_plane_envelopes_received_total{producer,verdict}` (counter, in-memory; resets at restart). The labelled receive counter lets a Prometheus scraper break down receive volume by source agent and verification outcome (PROVEN / VIOLATED / DEGRADED). Closes #25's "Prometheus-style federation" task.
...
```bash
lemma control-plane install --shape <systemd|docker-compose|helm|terraform|airgap> --output ./deploy [options]
```

| Option | Default | Applies to | Description |
|--------|---------|------------|-------------|
| `--shape` | (required) | all | `systemd` (a hardened unit running `control-plane serve`), `docker-compose`, `helm` (a Kubernetes Helm chart), `terraform` (an AWS module), or `airgap` (offline-bundle tooling). |
...
| `--image` | the published image | docker-compose, helm, terraform, airgap | Container image to run / bundle. |
