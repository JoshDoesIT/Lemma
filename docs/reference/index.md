...
The agent binary's source tree lives at [`agent/`](https://github.com/JoshDoesIT/Lemma/tree/main/agent). It exposes two subcommands:
...
`verify` walks a Lemma signed evidence JSONL, recomputes each entry's chain hash from the canonical `{event, provenance_excluding_storage}` payload, and checks the Ed25519 signature against the producer's public key under `<dir>/<producer>/<key_id>.public.pem`. With `--crl <path>` (repeatable, one per producer in a multi-producer bundle) it also flips a verdict from PROVEN to VIOLATED when the signing key was revoked at or before the envelope's `signed_at` — same `signed_at >= revoked_at` rule Python's `lemma evidence verify --crl` applies. CRLs whose `producer` field doesn't match the envelope's `event.metadata.product.name` are ignored for that envelope. Each CRL's own signature is verified against the issuer's public key up front; a bad CRL aborts the run with exit `1` before any per-envelope check runs. When no `--crl` is supplied the agent prints `Note: No CRL supplied; revocations issued elsewhere are not visible.` to stdout — exit code unchanged. Exit `0` means every entry is PROVEN; exit `1` means at least one entry is VIOLATED (chain, hash, signature, or CRL revocation) or a supplied CRL failed its own signature check; exit `2` is a usage error. The check is byte-for-byte parity with Python's `lemma evidence verify` envelope and CRL layers.

`sign` reads one OCSF event from `--event <path>` or stdin, resolves the producer's ACTIVE key from `<dir>/<producer>/meta.json`, and emits one signed envelope JSONL line on stdout. Defaults to genesis (`"0"*64`) when `--prev-hash` is omitted; operators chain envelopes by piping the prior entry's `entry_hash` into the next call. The output is byte-compatible with Python's `EvidenceLog.append` — Python verifies what Go signs and vice versa. **Caveat**: the input event must already be in the form Pydantic's OCSF models would emit (default fields like `severity_id`, `status_id`, `message` set explicitly), or the Python verifier will recompute a different canonical hash and reject the envelope. Pre-normalize through `lemma.services.ocsf_normalizer.normalize` if needed. Later slices add full producer-key lifecycle enforcement and federation. See [`agent/README.md`](https://github.com/JoshDoesIT/Lemma/blob/main/agent/README.md) for build instructions and the full feature roadmap under #25.
