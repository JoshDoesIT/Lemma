...
`keygen` mints an Ed25519 keypair for `--producer` under `--keys-dir` and writes the same on-disk layout Python's `crypto.generate_keypair` produces: `<safe_producer>/<key_id>.private.pem` (PKCS#8, mode 0o600), `<safe_producer>/<key_id>.public.pem` (SubjectPublicKeyInfo, 0o644), and `<safe_producer>/meta.json` with one ACTIVE record. The `key_id` derivation (`"ed25519:" + sha256(raw_public).hex()[:16]`) and `_safe_producer` rule (replace `/` and ` ` with `_`) match Python byte-for-byte, so a key minted by either side is loadable by both. **Idempotent** — a second call against a producer that already has an ACTIVE key returns its key_id without modifying any files. Pair with `ingest` and `verify` for a fully Go-only agent deployment. **Rotation and revocation are not in the agent binary yet**; use Python's `crypto.rotate_key` / `crypto.revoke_key` for those, or wait for later #25 slices.

`forward` POSTs each line of a signed-evidence JSONL to a Control Plane URL with `Content-Type: application/json`. The most common pairing is `ingest` → `forward`. `--header KEY=VALUE` is repeatable for `Authorization` or routing headers (Content-Type is set automatically and cannot be overridden). `--timeout` sets a per-request deadline in seconds. A 2xx response counts as forwarded; anything else (4xx, 5xx, transport error) counts as failed. Output: `<N> forwarded, <M> failed.` Exit `0` only when all forwarded; exit `1` if any failed. **HTTP/HTTPS only in this slice** — mTLS, retry/backoff, resumable forwarding, and the Control Plane receiver are tracked under #25. See [`agent/README.md`](https://github.com/JoshDoesIT/Lemma/blob/main/agent/README.md) for build instructions and the full feature roadmap under #25.
